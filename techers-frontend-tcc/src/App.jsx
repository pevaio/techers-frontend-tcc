...
export default function App() {
  ...
  const handleReset = () => {
    if (!confirm("Isso vai apagar todas as notas salvas e restaurar o padrão. Continuar?")) return;
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Roboto Serif, serif', backgroundColor: '#0b0b0b', color: '#fff' }}>
      ...
      <aside style={{ width: '260px', borderRight: '1px solid #3b3b3b', padding: '20px', boxSizing: 'border-box', backgroundColor: '#0b0b0b', display: 'flex', flexDirection: 'column' }}>
        <h3 style={{ margin: '0 0 15px 0', color: '#fff', textTransform: 'uppercase', fontSize: '12px', letterSpacing: '1px' }}>Notas Prontas</h3>
        ...
        <button
          onClick={handleReset}
          style={{ width: '100%', padding: '10px', cursor: 'pointer', marginTop: '8px', backgroundColor: 'transparent', color: '#f87171', border: '1px solid #f87171', borderRadius: '6px', fontWeight: '600' }}
        >
          Resetar Dados
        </button>
      </aside>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <header style={{ padding: '15px 25px', borderBottom: '1px solid #2d3748', backgroundColor: '#0b0b0b' }}>
          <h2 style={{ margin: 0, fontSize: '18px' }}>EDITANDO - <span style={{ color: '#008700', textTransform: 'uppercase' }}>{activeNoteTitle}</span></h2>
        </header>
        ...
        <div style={{ flex: 1, display: 'flex' }}>
          ...
          {/* PREVIEW */}
          <div className="markdown-preview" style={{ flex: 1, padding: '25px', overflowY: 'auto', lineHeight: '1.6' }}>
            <ReactMarkdown
